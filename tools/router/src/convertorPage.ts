    ...
    lookupBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          } else if (res.status === 530 || (res.status >= 521 && res.status <= 527)) {
            showErr(
              'HTTP ' +
                res.status +
                ': the worker could not reach CONVERTOR_ORIGIN (often missing DNS, wrong host, or convertor not bound to 0.0.0.0). Fix hosting, then redeploy or update Worker variables — not a yt-dlp/Python issue.'
            );
          } else {
            showErr(
              'Request failed (HTTP ' +
                res.status +
                '). If the convertor is reachable, check its terminal for yt-dlp/ffmpeg errors (this app uses Node + youtube-dl-exec; Python is only needed if you run yt-dlp directly).'
            );
          }
          return;
        }
