...
export function ExportModule({
  ...
  const downloadPdf = useCallback(async () => {
    ...
    try {
      ...
      // Page 9 — Shadow analysis
      pdf.addPage()
      pdf.setFont('times', 'bold')
      pdf.setFontSize(14)
      pdf.text('Shadow analysis', margin, 20)
      const shSummer = await tryPdfSvg('sonde-svg-shadow-range-summer')
      const shWinter = await tryPdfSvg('sonde-svg-shadow-range-winter')
      const shCalendar = await tryPdfSvg('sonde-svg-shadow-calendar')
      if (shSummer) addImageFit(shSummer, margin, 30, (pageW - 3 * margin) / 2, 85)
      if (shWinter) addImageFit(shWinter, margin + (pageW - 3 * margin) / 2 + margin, 30, (pageW - 3 * margin) / 2, 85)
      if (shCalendar) addImageFit(shCalendar, margin, 124, pageW - 2 * margin, 70)
      pdf.setFont('courier', 'normal')
      pdf.setFontSize(8.5)
      pdf.text('Key findings:', margin, 206)
      pdf.text('South facade: high summer exposure, reduced winter sun.', margin, 212)
      pdf.text('North face: typically shaded for long periods.', margin, 217)
      drawFooter()

      // Page 10 — Microclimate
      pdf.addPage()
      pdf.setFont('times', 'bold')
      pdf.setFontSize(14)
      pdf.text('Microclimate', margin, 20)
      const shHours = await tryPdfSvg('sonde-svg-sunlight-hours')
      const shSvf = await tryPdfSvg('sonde-svg-svf')
      const shSolar = await tryPdfSvg('sonde-svg-solar-radiation')
      if (shHours) addImageFit(shHours, margin, 30, 120, 90)
      if (shSvf) addImageFit(shSvf, margin + 130, 30, 120, 90)
      if (shSolar) addImageFit(shSolar, margin, 128, 120, 90)
      pdf.setFont('courier', 'normal')
      pdf.setFontSize(8.5)
      pdf.text('Use sunlight-hours heatmap for outdoor programme placement.', margin + 130, 138)
      pdf.text('SVF and daylight factor are indicative only; verify with detailed modelling.', margin + 130, 144, { maxWidth: 130 })
      drawFooter()
